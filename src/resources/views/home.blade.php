<x-app-layout>
    ...
    <div class="py-6 max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-6">
        ...
        $greeting = $hour < 12 ? 'Buenos días' : ($hour < 20 ? 'Buenas tardes' : 'Buenas noches' );
            ...
            {{-- GRID 2 COLUMNAS: RESERVAS + CLASES --}}
                ...
                <div style="background: #fff; border-radius: 12px; border: 0.5px solid #d4d9cc; padding: 24px;">
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 4px;">
                        ...
                        <a href="{{ route('player.reservations.create') }}" wire:navigate
                            style="font-size: 13px; color: #6b8f6b; font-weight: 500; text-decoration: none;">+ Nueva</a>
                    </div>
                    <p style="font-size: 26px; font-weight: 600; color: #2d3b2d; margin: 0 0 16px;">
                        {{ $playerData['monthReservationCount'] }}
                        <span style="font-size: 13px; font-weight: 400; color: #7a8a7a;">este mes</span>
                    </p>
                    @forelse($playerData['upcomingReservations'] as $res)
                    <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px 0; border-bottom: 0.5px solid #f0f3ee;">
                        <div style="min-width: 0;">
                            <p style="font-size: 13px; font-weight: 500; color: #2d3b2d; margin: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $res->court->name }}</p>
                            <p style="font-size: 11px; color: #7a8a7a; margin: 2px 0 0;">
                                {{ \Carbon\Carbon::parse($res->reservation_date)->format('d/m') }}
                                · {{ \Carbon\Carbon::parse($res->start_time)->format('H:i') }}–{{ \Carbon\Carbon::parse($res->end_time)->format('H:i') }}
                            </p>
                        </div>
                        <div style="text-align: right; flex-shrink: 0; margin-left: 10px;">
                            <p style="font-size: 13px; font-weight: 600; color: #6b8f6b; margin: 0;">{{ number_format($res->total_price, 2) }}€</p>
                    ...
                    @empty
                    <p style="font-size: 13px; color: #9aaa9a; margin: 0;">No tienes reservas próximas.</p>
                    @endforelse
                    @if($playerData['upcomingReservations']->isNotEmpty())
                    <a href="{{ route('player.reservations.index') }}" wire:navigate
                        style="display: block; text-align: center; font-size: 12px; color: #6b8f6b; font-weight: 500; text-decoration: none; margin-top: 12px;">
                        Ver todas →
                    </a>
                    @endif
                </div>
                ...
                <div style="background: #fff; border-radius: 12px; border: 0.5px solid #d4d9cc; padding: 24px;">
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 4px;">
                        ...
                        <a href="{{ route('player.classes.index') }}" wire:navigate
                            style="font-size: 13px; color: #6b8f6b; font-weight: 500; text-decoration: none;">Ver todas</a>
                    </div>
                    <p style="font-size: 26px; font-weight: 600; color: #6b8f6b; margin: 0 0 16px;">
                        {{ number_format($playerData['monthSpent'], 2) }}€
                        <span style="font-size: 13px; font-weight: 400; color: #7a8a7a;">gastado este mes</span>
                    </p>
                    @forelse($playerData['upcomingClasses'] as $class)
                    <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px 0; border-bottom: 0.5px solid #f0f3ee;">
                        <div style="min-width: 0;">
                            <p style="font-size: 13px; font-weight: 500; color: #2d3b2d; margin: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $class->title }}</p>
                            <p style="font-size: 11px; color: #7a8a7a; margin: 2px 0 0;">
                                {{ \Carbon\Carbon::parse($class->date)->format('d/m') }}
                                · {{ \Carbon\Carbon::parse($class->start_time)->format('H:i') }}
                                · {{ $class->coach->name }}
                            </p>
                        </div>
                        <p style="font-size: 13px; font-weight: 600; color: #6b8f6b; margin: 0; flex-shrink: 0; margin-left: 10px;">{{ number_format($class->price, 2) }}€</p>
                    </div>
                    @empty
                    <p style="font-size: 13px; color: #9aaa9a; margin: 0;">No tienes clases próximas programadas.</p>
                    @endforelse
                </div>
